...
/**
 * @defgroup sdi_i2c_flags I2C Flag, for ex: SMBUS PEC support
 * @{
 */
/**
...
/**
 * @enum sdi_i2c_operation_t
 * SDI I2C Bus Read/Write Operation
 */
typedef enum {
    /**
     * SDI I2C BUS WRITE OPERATION
     */
    SDI_I2C_WRITE,
    /**
     * SDI I2C BUS READ OPERATION
     */
    SDI_I2C_READ,
} sdi_i2c_operation_t;
...
/**
 * @typedef sdi_i2c_bus_capability_t
...
typedef struct sdi_i2c_bus_ops {
    ...
    t_std_error (*sdi_smbus_execute) (sdi_i2c_bus_hdl_t bus, sdi_i2c_addr_t address,
     ...
     /**
     * @brief sdi_i2c_execute
     * Execute I2C Transaction on
     * - bus : I2C Bus
     * - address : Slave Address
     * - operation : Read/Write Operation
     * - cmd : list of read/write offsets
     * - cmdlen : no. of offsets
     * - buffer : Data to read from/write to of I2C Slave
     * - buflen : no.of bytes to read/write
     * - flags : options if any to be send to i2c execute
     */
    t_std_error (*sdi_i2c_execute)(sdi_i2c_bus_hdl_t bus, sdi_i2c_addr_t address,
                                   sdi_i2c_operation_t operation,
                                   const uint8_t *cmd, uint_t cmdlen,
                                   void *buffer, uint_t buflen, uint_t flags);
    /**
...
/**
 * @brief sdi_i2c_execute
 * Wrapper for i2c execute bus sdi_i2c_execute
 * @param[in] bus_handle - i2c bus handle
 * @param[in] address - i2c slave address
 * @param[in] operation - read/write operation
 ...
 * @return STD_ERR_OK on Success, SDI_I2C_ERRNO on Failure
 */
static inline t_std_error sdi_i2c_bus_execute (sdi_i2c_bus_hdl_t bus_handle,
                          sdi_i2c_addr_t address,
                          sdi_i2c_operation_t operation, const uint8_t *cmd,
                          uint_t cmdlen, void *buffer, uint_t buflen, uint_t flags)
{
    return bus_handle->ops->sdi_i2c_execute(bus_handle, address, operation,
